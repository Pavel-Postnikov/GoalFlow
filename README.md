...
│   └── seed.ts           # Демо-данные: 3 цели, 5 проектов, 20 задач (только dev)
├── store/
...
В dev-режиме (`npm run dev`) в самом низу сайдбара появляется кнопка **«Загрузить демо-данные»** (иконка колбы). Она заполняет IndexedDB реалистичными данными:

- 3 цели: «Запустить продукт», «Личный рост», «Здоровье и спорт»
- 5 проектов с задачами в разных статусах (todo / in_progress / done)
- Задачи с приоритетами, дедлайнами и временем трекинга
- 3 задачи в Inbox

Удобно для ревью преподавателя, демонстрации заказчику или первого знакомства с проектом — не нужно добавлять данные вручную. Кнопка видна **только в dev-режиме** и отсутствует в production-сборке на Vercel.
